...
import React, { type FC, useState, Fragment, useMemo, useCallback, useRef } from 'react';
import {
  EuiWrappingPopover,
  EuiListGroup,
  EuiListGroupItem,
...
function ManagedFlyout({
  ...
  }, [exportIntegration.config, intl, usePrintLayout]);
...
function ExportMenuPopover({ intl }: ExportMenuProps) {
  ...
  const selectedMenuItem = useMemo(() => {
    ...
    return selectionOptions.current[selectedMenuItemMeta.group].find(
      (item) => item.id === selectedMenuItemMeta.id
    )!;
  }, [selectedMenuItemMeta]);

  const flyoutOnCloseHandler = useCallback(() => {
    return setIsFlyoutVisible(false);
  }, []);
          ...
          {exportIntegrations.map((menuItem) => (
            ...
            >
              <EuiListGroupItem
                ...
                onClick={() => {
                  setSelectedMenuItemMeta({ id: menuItem.id, group: 'export' });
                    ...
                    {React.cloneElement(exportDerivative.config.label, {
                      onClick: () => {
                        setSelectedMenuItemMeta({
                          id: exportDerivative.id,
                          group: 'exportDerivatives',
                        });
                        setIsFlyoutVisible(true);
                      },
                    })}
